...
class SMSAPIClient {
    ...
    func networking<T:Codable>(from api: SMSAPI) -> Observable<T> {
        Observable.create { (obs) -> Disposable in
            let request = Alamofire.request(URL(string: api.baseURL + api.path)!, method: api.method, parameters: ["":""], encoding: api.encoding, headers: api.header).responseData { (response) in
